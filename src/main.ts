...
scene.fog = new THREE.Fog(0x0a0805, 6, 28);
...
scene.add(new THREE.AmbientLight(0xffeacc, 0.35));
const torch = new THREE.PointLight(0xffc070, 1.4, 14, 1.5);
scene.add(torch);
const ceilingLight = new THREE.PointLight(0x88aaff, 0.4, 30, 1);
ceilingLight.position.set(0, WALL_HEIGHT - 0.2, 0);
...
for (const ep of level.enemySpawns) spawnEnemy(scene, ep);
...
renderer.domElement.addEventListener('click', () => {
  ...
  if (def.ammo <= 0) {
    if (weapons.cooldown <= 0) audio.playEmpty();
    return;
  }
...
renderer.setAnimationLoop(() => {
  ...
  updatePlayer(player, keys, dt, yaw);
  world.step();
